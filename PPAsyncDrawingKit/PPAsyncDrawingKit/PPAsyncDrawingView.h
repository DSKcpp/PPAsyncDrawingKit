...
/**
 设置异步绘制所在的线程，如果不设置，并且绘制策略是多线程的话，内部会自行开辟一个线程进行绘制
 */
@property (nullable, nonatomic, assign) dispatch_queue_t drawQueue;

/**
 默认是 NO，如果绘制策略是 None 的话，将有这个属性来判断是否异步绘制，一般在滚动的时候设为 YES，进行点击事件时设置为 NO，
 防止出现闪烁，YES 表示使用异步绘制
 */
...
/**
 表示一个 View 绘制的次数，自增，用来打断快速滑动时的绘制，大幅提高性能
 */
